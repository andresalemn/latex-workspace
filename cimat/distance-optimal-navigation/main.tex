...
\begin{frame}{3B. Motion Primitives}
    \begin{itemize}
...
\begin{frame}{Section IV.A: GNT Structure \& Vertices}
    \begin{itemize}
...
\begin{frame}{Mini-Glossary: GNT Formalism}
	\begin{description}[labelwidth=-0.8cm]
		...
		\item[\descmath{I, J}] Connected segments of the boundary $\partial R$.
		\item[\descmath{L}] A line that partitions space or acts as a tangent.
		\item[{Gen. Inflection}] Where boundary $I$ crosses its tangent line $L$.
		\item[{Gen. Bitangent}] A line $L$ tangent to two disjoint segments $I$ and $J$.
	\end{description}
\end{frame}


% --- Slide: IV.B & C Critical Events & Geometry ---
\begin{frame}{IV.B \& C: Critical Events \& Geometric Triggers}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            \small
            \textbf{The Four Events (Fig 5):}
            \begin{enumerate}
            ...
            \end{enumerate}
            \vspace{0.2cm}
            \textbf{Geometric Relations (Fig 6):}
            \begin{itemize}
                \item \textbf{Inflections:} Line $L$ partitions $I$ into $I_1, I_2$. Triggers Appearance/Disappearance.
                \item \textbf{Bitangents:} Line $L$ is tangent to disjoint $I$ and $J$. Triggers Merge/Split.
            \end{itemize}
        \end{column}
        \begin{column}{0.4\textwidth}
            \begin{center}
                \textit{[Insert Figure 5 \& 6]}
            \end{center}
            \footnotesize
            \textbf{Lemma 1:} Merged gaps $g_1, g_2$ always reappear at the same angle when their parent splits.
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Section IV.D: The Gap-Based Roadmap ($S$)}
    \begin{itemize}
        ...
        \item \textbf{Function:} It serves as a topological "skeleton" of the environment.
        \item \textbf{Termination (Lemma 2):} Every $\text{chase}(g)$ is guaranteed to terminate because the robot must eventually cross an inflection or bitangent.
    \end{itemize}
    \vfill
    \begin{center}
        \textit{$S$ contains all information needed for distance-optimal navigation.}
    \end{center}
\end{frame}

% --- Slide: IV.E Constructing a Complete GNT ---
\begin{frame}{Section IV.E: Exploration Strategy}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            \textbf{The Algorithm:}
            \begin{enumerate}
            ...
            \vspace{0.3cm}
            \textbf{Lemma 3:} 
            This process is guaranteed to finish because $\partial R$ has a finite number of inflection and bitangent triggers.
        \end{column}
        \begin{column}{0.4\textwidth}
            \begin{center}
                \textit{[Insert Figure 8]} \\
                \footnotesize \textbf{Figure 8: Incremental construction of the GNT.}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
